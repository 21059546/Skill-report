...
public class LowPolyTerrain : MonoBehaviour
{
	...
	void Start()
	{
		...
		var v1 = Vector3.zero;                                       // (0,0,0)
		var v2 = Vector3.forward * _terrainSize;                     // (0,0,x)
		var v3 = new Vector3(_terrainSize, 0, _terrainSize);         // (x,0,x)
		var v4 = Vector3.right * _terrainSize;                       // (x,0,0)
		_trisList.Add(new Tr(v1, v2, v3));
		_trisList.Add(new Tr(v1, v3, v4));

		while (_c < MaxTriangles)
		{
			var t = new List<Tr>();
		...
		GetComponent<MeshCollider>().sharedMesh = _mesh;              // 要检索的组件的类型。

		CalculateHeightMap();
		
		Terrain.gameObject.SetActive(false);                                         // Activate or deactivate the object
	}
	...
	private void CalculateHeightMap()
	{
		...
		for (var k = 0; k < _trisList.Count; k++)
		{
			...
			var ki = Mathf.RoundToInt((x1 + x2 + x3) / 3);                     // 返回舍入到最接近的整数。f       如果数字以 .5 结尾，因此它位于两个整数之间，其中一个是偶数，另一个是奇数，则返回偶数。
	...
	{
		if (Vector3.Dot(n, Vector3.Cross(t.V2 - t.V1, t.V3 - t.V1)) < 0)                       // 三个向量的点积
		{
			var v3 = t.V3;
			t.V3 = t.V2;
			t.V2 = v3;
